...
<head>
  ...
  <nav class="navbar navbar-expand-sm navbar-dark bg-dark">
           <a class="navbar-brand" href="class.php?userid=<?php echo $_GET['userid']?>">
              ...
              <div class="topnav">
                <a class="nav-item" href="assignment.php?class_id=<?php echo $_GET['id'];?>&&userid=<?php echo $_GET['userid'];?>">Bài tập trên lớp</a>
                <a href="classlist.php?id=<?php echo $_GET['id'];?>&&userid=<?php echo $_GET['userid'];?>">Danh sách lớp</a>
              </div>
            <ul></ul>
        </nav>
    <!--Form comment-->
                    ...
                    <div class="card mb-3">
                        ...
                        <div class="card-img-overlay">
                            ...
                            <?php
                                ...
                                $sql = "SELECT class_name, `class_id` from classes where `class_id` = '$class_id'";
    
                                $result = mysqli_query($connection, $sql) or die(mysqli_error($connection));
                                $row = mysqli_fetch_array($result);

                                echo "<p class=\"card-text text-light\">$row[0]</p>";
                                echo "<p class=\"card-text text-light\">Mã số lớp học: $row[1]</p>";
                            ?>
                        </div>
                    </div>
        </div>
